...
public abstract class ZclAppliance extends DriverAppliance implements IZclAppliance, ZigBeeDeviceListener {
	...
	public synchronized void attach(IHacDevice device) {
		...
		try {
			serviceEndPoint = this.zclGetEndPoint(epsNumber, profileId, deviceId, endPointId, 0, service.getSimpleDescriptor().getApplicationOutputCluster(), service
					.getSimpleDescriptor().getApplicationInputCluster());
	...
	public synchronized void detach(IHacDevice device) throws ApplianceException {
		...
		for (int i = 0; i < serviceEndPoints.length; i++) {
			...
			if (serviceEndPoint instanceof ZclEndPoint && (attachedDevice = ((ZclEndPoint) serviceEndPoint).zclGetZigBeeDevice()) != null
	...
	protected ZclEndPoint zclGetEndPoint(int epsNumber, int profile_id, int device_id, int end_point_id, int manufacturer_code, final List clientClusterIds,
			final List serverClusterIds) throws ApplianceException {
